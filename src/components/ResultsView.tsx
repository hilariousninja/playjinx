...
export default function ResultsView({ promptId }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {userStat && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="game-card text-center">
      ...
      <div className="game-card">
        ...
        <div className="space-y-2">
          {stats.slice(0, 8).map((s, i) => (
            <motion.div key={s.normalized_answer} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center gap-3">

...
import { Users, UserPlus, Radio, Plus, Loader2, X, ArrowRight } from 'lucide-react';
...
/**
 * Shows the user's active group(s) as a destination card — not just a share CTA.
 * - 0 groups → compact "Start a group" card
 * - 1 group → group context card with View + Invite
 * - 2+ groups → stacked list of group cards
 */
export default function ActiveGroupCard({ className = '' }: Props) {
  ...
  const handleInvite = async (g: GroupWithActivity) => {
    ...
    return (
      ...
      >
        ...
        <div className="text-left min-w-0 flex-1">
          ...
          <p className="text-[10px] text-muted-foreground/50 leading-tight">Play daily with friends — no new links needed</p>
        </div>
      </button>
    );
  }
  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      {groups.slice(0, 3).map(g => (
        <div key={g.id} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-border/50 bg-card hover:border-primary/20 transition-all group">
          <Link to={`/g/${g.invite_code}/today`} className="flex items-center gap-3 min-w-0 flex-1">
            <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              ...
              className="p-1.5 rounded-lg text-muted-foreground/30 hover:text-primary/60 hover:bg-primary/5 transition-colors"
            ...
            <Link
              ...
              className="p-1.5 rounded-lg text-muted-foreground/20 hover:text-primary/40 transition-colors"
            >
